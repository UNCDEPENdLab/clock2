...
for (iT in 1:nT-1){
  if (iT==1){
    df0 <- paste0('\t list.rt_',as.character(df_t$ix[iT]),'.currentindex=51; \n')
  } else if (iT > 1){
    df0 <- paste0(df0,'\t list.rt_',as.character(df_t$ix[iT]),'.currentindex=51; \n')
  }
}
iqx_formatted_df[1,1] <- df0 
write.table(iqx_formatted_df,'listupdate51',row.names=F,col.names=F,quote=F)
